...
\begin{table}[htpb]
  ...
  \caption{Distribution of ground truth annotations across the different element types contained in the \lstinline!publaynet-mini! subset of the PubLayNet dataset. Element types are disproportionally represented, with ``Text'' making up a large majority of the dataset.}\label{tab:publaynet_counts}
\end{table}
...
\begin{figure}[htbp]
  ...
  \hfill
  \caption{A pair of identical bounding boxes at different \gls{iou} values. The shaded area is the intersection of the bounding boxes. As the \gls{iou} value increases, the area of the intersection approaches the area of the bounding box.}
\end{figure}
...
\begin{equation}
  \text{F1}_\text{weighted} = \sum_{i=1}^{C}w_i\text{F1}_i
  \label{eq:weightedf1}
\end{equation}
...
\paragraph{Dataset creation:} To compare the chunking strategies against each other, a corpus of oncology guideline documents needs to be selected for the evaluation. In addition, \gls{qa} pairs are needed to evaluate the quality of the retrieval. Hereby the framework provides utilities for the generation of synthetic \gls{qa} pairs from the document corpus through the use of a \gls{llm} \autocite{chroma_eval}. For our evaluation we use manually annotated \gls{qa} pairs, created by medical professionals from the \gls{tum} university hospital. While this dataset is still in development and will not be available to the general public, we were permitted to use both the documents as well as the \gls{qa} pairs in their current state for this evaluation. In total, the dataset contains 46 \gls{qa} pairs created from a document corpus of eleven german oncology guidelines published by \gls{awmf}.
...
\begin{enumerate}
  ...
  \item \textbf{Heading token budget ($B_h$):} In hierarchical chunking, the heading budget prevents section headers from taking up a majority of the chunk's content. For our evaluation, we set $B_h=0.4\cdot N$.
\end{enumerate}

In total, we evaluate 24 different chunking configurations across four different strategies.

% In order to not only be able to compare different chunking strategies against each other but also enable comparisons between chunking strategy configurations, we evaluate each chunking strategy for a range of possible parameters. In order to enable a thorough investigation while keeping the number of tested configurations manageable, we follow existing literature to identify reasonable value ranges for these parameters.

% The maximum chunk length is the most important parameter. Previous studies found that the chunk size has a significant effect on the efficiency of the retrieval

% There is no universally accepted best value for the maximum chunk size

% While the evaluated strategies are already established, there are a multitude of parameters which need to be set for the evaluation. There is no universally accepted best value for these parameters, with the value depending on the specific data and information density of the application. Additionally, there has been little scientific research on the effect of these parameters, especially for a multitude of chunking strategies. In order to evaluate a broad range of configurations, while keeping the number of tested configurations manageable, we will evaluate each chunker on four maximum token limits $N\in\{128,256,512,1024\}$. For the chunkers using a sliding window approach, we will follow the

% We will therefore follow the documentation from the \gls{rag} framework LlamaIndex, which suggests starting with a chunk size
% While there is no ``one size fits all'' approach for setting the maximum chunk size, \todo{source} suggest starting with a chunk size of around 250 for general applications. Other experiments have used chunk sizes ranging from

% Deciding on what configurations to test the chunking strategies on is not trivial.

% For our evaluation

% 3 token lengths: small 128, medium 256, large 512, xl 1028
% 3 overlaps: 50\%, 25% and 0\%
