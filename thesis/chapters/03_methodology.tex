...
\begin{enumerate}
  ...
  \item \textbf{Chunk overlap ($O$):} While sliding-window approaches are common in practice, empirical research on the specific effects of overlap remains sparse. Configurations in the literature vary drastically, from the variable $0.5\cdot N$ experimented with by \textcite{chroma_eval} to fixed smaller sizes used by \textcite{semantic_chunking} and LlamaIndex, with the latter setting an unusually small default overlap of 20 tokens on their chunking implementations. In order to strike a balance between these configurations and test the impact of including an overlap, we evaluate the strategies at $O\in\{0,0.20\cdot N\}$.
  ...
  \item \textbf{Minimum chunk size ($M$):} To ensure that semantic chunks scale with the maximum chunk size, the minimum chunk size needs to scale accordingly as well. We therefore set $M=0.5\cdot N$.
