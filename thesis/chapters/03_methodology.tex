...
A \gls{dtbb} is classified as a \gls{tp} if there exists a \gls{gtbb} from the same class such that their \gls{iou} is greater than a given threshold. One \gls{gtbb} can not be matched to multiple \glspl{dtbb}. If there does not exist a \gls{gtbb} that fulfils these criterions, the \gls{dtbb} is classified as a \gls{fp}. Any \glspl{gtbb} which were not matched to a \gls{dtbb} are classified as \glspl{fn}. Following the definition from \textcite{maximize_f1}, for a class $i$, precision and recall are formulated as described in \autoref{eq:precision} and \autoref{eq:recall}. Hereby $tp_i$, $fp_i$, and $fn_i$ denote the count of their respective classification for bounding boxes of class $i$.
