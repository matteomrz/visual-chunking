...
\textbf{Document quality:} Depending on the \gls{cpg}'s age and publishing organization, the formatting of the document may contain significant structural errors. Observed formatting issues include overlapping text, empty pages between content, tables extending into the page margins, and invisible text on document pages.
...
\section{Chunking}
Chunking as a terminology has emerged with the emergence of the \gls{rag} paradigm. The term refers to the splitting of longer structured documents into smaller atomic units called chunks \autocite{rag_survey,chunk_size_effect_on_rag}. While Chunking is the pre

\todo{Other usages of chunking than RAG}

\begin{itemize}
  \item Chunking is also called text segmentation
  \item close resemblance to passage-based document retrieval
  \item refers to the process of splitting up \"documents\", structured data, text\dots
  \item find all applications. Especially for the rag paradigm\dots
  \item Types of chunking / text segmentation / text splitting
  \item difference to text splitting -> we may have access to more metadata about the text: elements, relationships
\end{itemize}
