...
def _get_json(exist_ok: bool = False) -> list:
    ...
    if exist_ok and final_path.exists():
        print(f"OmniDocBench.json already exists at: {final_path}")
    else:
        hf_hub_download(
...
def _get_images(omni_doc, exist_ok: bool = False) -> list[str]:
    ...
    for instance in omni_doc:
        try:
            ...
            print(f"Error: OmniDocItem is malformed: {str(e)}")
...
def _images_to_pdfs(exist_ok: bool = False):
    ...
    for image_path in image_dir.glob(pattern="*.jpg"):
        ...
        if exist_ok and pdf_path.exists():
            print(f"Skipping existing PDF for: {image_path.stem}")
        else:
            try:
                pdf = pymupdf.open()
                ...
                pdf.save(pdf_path)
                pdf.close()
                print(f"Successfully converted: {image_path.stem}")
            except Exception as e:
                print(f"Error: Failed converting: {image_path.stem}, due to: {str(e)}")
...
def create_config_files(exist_ok: bool = False):
    ...
    for parser in Parsers:
        ...
        if exist_ok and p_path.exists():
            print(f"Skipping existing config file for method: {p_name}")
            continue

        p_content = template.replace("{{method_name}}", p_name)
        with open(p_path, "w") as c:
            c.write(p_content)
            print(f"Created config file for method: {p_name}")
...
def prepare_omni_doc_bench(exist_ok: bool = False):
    ...
    print("Finished setup for OmniDocBench. For usage check the README.md.")
